...
from utils.utils_or import get_subwindow_tracking
...
def generate_anchor(total_stride, scales, ratios, score_size):
    ...
    for ratio in ratios:
        ...
        for scale in scales:
            ...
            anchor[count, 3] = hhs
            count += 1
    #print(anchor.shape,score_size)
    anchor = np.tile(anchor, score_size * score_size).reshape((-1, 4))
    ori = - (score_size / 2) * total_stride
    #print(ori,score_size)
...
class TrackerConfig(object):
    ...
    total_stride = 8
    score_size = (instance_size-exemplar_size)/total_stride+1 # calculate the feature size, exemplar_size is the kernel size
...
def SiamRPN_track(state, im):
    ...
    #s_z = 188.0
    print("s_z:",s_z)
    scale_z = p.exemplar_size / s_z # exemplar_size 是规定输入的size
    print("scale_z:",scale_z)
    print("exemplar_size:",p.exemplar_size)
    print("instance_size",p.instance_size)
    ...
    #从原图切s_x大小的图片，缩放到instance_size大小
    '''
