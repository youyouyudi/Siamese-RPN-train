...
def get_labels_bbox_targets(ispositive, label_pos,label_sz,anchors):
    ...
    for n in range(len(anchors_rect)):
        ...
        ious[n] = iou
    inds_inside = np.where(ious > 0.45)[0]
    ...
    bbox_targets[inds_inside] = bbox_transform_inv(anchors[inds_inside],label_pos,label_sz)#label_pos,label_sz are for the patch
    bg_inds_inside = np.where(ious < 0.15)[0]
    #print("len(bg_inds_inside)", len(bg_inds_inside))
    num_bg = 50
    if len(bg_inds_inside) > num_bg:
        #enable_inds = bg_inds_inside[npr.randint(len(bg_inds_inside), size=num_bg)]
        enable_inds = random.sample(bg_inds_inside.tolist(), num_bg)
        #print("enable_inds", enable_inds)
...
def Secondframe_init(ispositive, state,im,target_pos2,target_sz2,optimizer,patch_scale=1):
    ...
    '''
    for i in range(output_tmp.shape[0]):
        print("&&&&&&&&")
        anchor = output_tmp[i]
        res = [0]*4
        res[0] = anchor[2] * bbox_targets_tmp[i, 0] + anchor[0]
        res[1] = anchor[3] * bbox_targets_tmp[i, 1] + anchor[1]
        res[2] = anchor[2] * math.exp(bbox_targets_tmp[i, 2])
        res[3] = anchor[3] * math.exp(bbox_targets_tmp[i, 3])

        #res = output_tmp[i]
        print("res",res)
        x1 = int(res[0] - 0.5 * res[2] + p.instance_size / 2)
        y1 = int(res[1] - 0.5 * res[3] + p.instance_size / 2)
        x2 = int(res[0] + 0.5 * res[2] + p.instance_size / 2)
        y2 = int(res[1] + 0.5 * res[3] + p.instance_size / 2)
        print("x1,y1,x2,y2", x1, y1, x2, y2)
        x1 = int(-76+ p.instance_size / 2)
        y1 = int(-76+ p.instance_size / 2)
        x2 = int(68+ p.instance_size / 2)
        y2 = int(68+ p.instance_size / 2)
        print("x1,y1,x2,y2",x1,y1,x2,y2)
        draw = cv2.rectangle(im_patch, (x1, y1), (x2, y2), (255, 0, 0), 2)
        cv2.imshow('result'+str(i)+'.jpg', draw)
        cv2.waitKey(0)
    '''
    #print("label_pos, label_sz", label_pos, label_sz)
